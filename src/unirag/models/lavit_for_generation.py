...
class LaVITforGeneration(nn.Module):
        ...
        **kwargs,
        ...
        if load_tokenizer:
            ...
            weight_path = os.path.join(
                model_path, "visual_tokenizer", "tokenizer_encoder.bin"
            )
            state_dict = torch.load(weight_path, map_location="cpu")
            quantize_dict = OrderedDict(
                {"embedding.weight": state_dict["quantize.embedding.weight"]}
            )
    ...
    def generate_image_tokenids(
        ...
        if not is_token_prompt:
            ...
            prompt_token_ids = (
                torch.ones((batch_size, max_length), dtype=torch.long).to(device)
                * self.llama_tokenizer.pad_token_id
            )
            prompt_attn_mask = torch.zeros(
        ...
        image_start_attn = torch.ones((batch_size, 1), dtype=torch.long).to(
            device
        )  # [batch_size, 1]
    ...
    def generate_image_embeds(self, image_tokens):
        ...
        if self.visual_tokenizer is None:
            ...
            token_quantize = self.visual_tokenizer.quantize.embedding(
                tokens_prune
            )  # [np, d]
    ...
    def run_safety_checker(self, image_array):
        safety_checker_input = self.feature_extractor(
            self.numpy_to_pil(image_array), return_tensors="pt"
        ).to(self.device)
    ...
    def pixel_decoding_origin(
        ...
        with self.maybe_autocast():
            ...
            encoder_hidden_uncond = torch.zeros(
                batch_size, num_tokens, C, dtype=xrec.dtype
            ).to(torch_device)
            uncond_embeddings = self.uncond_embeddings[0].to(xrec.dtype)
            encoder_hidden_uncond[:, : len(uncond_embeddings)] = uncond_embeddings

            # To set the mask
            encoder_mask = torch.ones(batch_size, num_tokens, dtype=torch.long).to(
                torch_device
            )
            uncond_encoder_mask = torch.zeros(
                batch_size, num_tokens, dtype=torch.long
            ).to(torch_device)
            ...
            for t in tqdm(self.scheduler.timesteps):
                ...
                noise_pred = noise_pred_uncond + guidance_scale_for_decoder * (
    ...
    def generate_image(
        ...
        with self.maybe_autocast():
            ...
            prompt_embeds, pooled_prompt_embeds = self.generate_image_embeds(
                image_tokens
            )
            ...
            add_time_ids = self._get_add_time_ids(
                original_size, crops_coords_top_left, target_size, prompt_embeds.dtype
            )
    ...
    def multimodal_synthesis(
        ...
        for prompt_str, prompt_type in prompts:
            assert prompt_type in [
                "image",
                "text",
            ], "The prompt type should be image or text"
            if prompt_type == "text":
                text_tokens = (
            ...
            if prompt_type == "image":
                ...
                with self.maybe_autocast():
                    image_tokens = self.visual_tokenizer.tokenize_image(
                        image_tensor, add_special=False
                    )[0]
