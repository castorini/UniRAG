...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, codebook_embeds, codebook_mask):
        ...
        q = (
            self.query(x)
            .reshape(B, N_x, self.num_heads, C // self.num_heads)
            .permute(0, 2, 1, 3)
        )
        k = (
            self.key(codebook_embeds)
            .reshape(B, N, self.num_heads, C // self.num_heads)
            .permute(0, 2, 1, 3)
        )
        v = (
            self.value(codebook_embeds)
            .reshape(B, N, self.num_heads, C // self.num_heads)
            .permute(0, 2, 1, 3)
        )
...
class AttentionPool2d(nn.Module):
    def __init__(
        self, seq_len: int, embed_dim: int, num_heads: int, output_dim: int = None
    ):
        super().__init__()
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.unet_attnpool = AttentionPool2d(
            num_patches, self.decoder_out_dim, num_heads, 1280
        )
...
def build_tokenizer_decoder(model_path="", pixel_decoding="highres"):
    if pixel_decoding == "lowres":
        ...
        weight_path = os.path.join(
            model_path, "visual_tokenizer", "tokenizer_decoder.bin"
        )
    else:
        model = HighresVQDecoder(depth=12)
        weight_path = os.path.join(
            model_path, "visual_tokenizer", "highres_tokenizer_decoder.bin"
        )
