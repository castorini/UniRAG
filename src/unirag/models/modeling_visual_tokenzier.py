...
class CausalFuserBlock(nn.Module):
    ...
    def __init__(
        ...
        attn_drop=0.0,
        act_layer=nn.GELU,
        norm_layer=partial(FusedLayerNorm, eps=1e-5),
    ):
        super().__init__()
...
class DynamicVisualTokenizer(nn.Module):
    ...
    def encode_features(self, x):
        ...
        pred_score = self.token_predictor(
            encoder_features.to(torch.float32), mask
        ).reshape(B, -1, 2)
        # Sample from the score distribution
        hard_keep_decision = F.gumbel_softmax(pred_score, hard=True)[
            :, :, 0
        ]  # [N, num_patches]
    ...
    def tokenize_image(self, x_tensor, add_special=False, used_for_llm=True):
        ...
        to_quantizer_features = self.encode_task_layer(
            remained_token.type_as(self.encode_task_layer[-1].weight)
        )
        ...
        if add_special:
            ...
            output_embed_ind = []
            image_special = torch.as_tensor([32000, 32001], dtype=torch.long).to(
                x_tensor.device
            )
            for ele in embed_ind_list:
                output_embed_ind.append(
                    torch.cat([image_special[:1], ele, image_special[1:]])
                )
...
def build_dynamic_tokenizer(
    ...
    if for_understanding:
        ...
        visual_weight_path = os.path.join(
            model_path, model_sub_dir, "visual_weight.bin"
        )
        print(
            f"For multi-modal understanding, Load visual tokenizer weight from {visual_weight_path}"
        )
